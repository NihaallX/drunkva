...
export async function POST(req: Request) {
  try {
    ...
    const notifTitle = `${taggerName} tagged you as a witness`;
    ...
    console.error("POST /api/witnesses failed", err);
    return NextResponse.json({ error: "Failed to tag witnesses" }, { status: 500 });
  }
}
...
export async function PATCH(req: Request) {
  try {
    ...
    if (err instanceof Response) return err;
    console.error("PATCH /api/witnesses failed", err);
    return NextResponse.json({ error: "Failed to update witness" }, { status: 500 });
  }
}
